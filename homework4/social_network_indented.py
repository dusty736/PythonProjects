...
# Opens the data file, and saves the data as integers into a list.
facebook = nx.Graph()
facebook_txt = open("facebook-links.txt", "r")
facebook_data = []
for line_of_text in facebook_txt:
    line = line_of_text.split("\t")
    for data in range(len(line) - 1):
        line[data] = int(line[data])
    facebook_data.append(line)
facebook_txt.close()
 
# Deletes the column of data with the Unix Timestamp.
for row in facebook_data:
    del row[2]
 
# Draws the graph, and saves the IDs to a set.
name_set = set()
ID_set_sorted = set()      
for pair in facebook_data:
    for userID in pair:
        facebook.add_edge(pair[0],pair[1])
        name_set.add(userID)
ID_set_sorted = sorted(name_set)
...
# Creates a list of the IDs divisible by 1000.
ID_by_1000 = []
for userID in ID_set_sorted:
    if userID % 1000 == 0:
        ID_by_1000.append(userID)
 
print "Problem 6:"
print ""
 
# Save the ID into a dictionary with the values saved being the first
# 10 recommendations based on the recommend by common friends function,
# and finally prints the user ID, and the list of recommendations.  
ID_common_rec = {}
for userID in ID_by_1000:
    common_fb = recommend_by_number_of_common_friends(facebook, userID)[0:10]
    ID_common_rec[userID] = common_fb
    print userID, "(by number_of_common_friends):", ID_common_rec[userID]                        
 
print ""
 
assert ID_common_rec[28000] == \
    [23, 1445, 4610, 7996, 10397, 11213, 56, 85, 471, 522]
assert ID_common_rec[29000] == \
    [28606]
assert ID_common_rec[30000] == \
...
# Save the ID into a dictionary with the values saved being the first
# 10 recommendations based on the recommend by influence function,
# and finally prints the user ID, and the list of recommendations.  
ID_influence_rec = {}
for userID in ID_by_1000:
    influence_fb = recommend_by_influence(facebook, userID)[0:10]
    ID_influence_rec[userID] = influence_fb
    print userID, "(by influence):", ID_influence_rec[userID] 
 
print ""      
 
assert ID_influence_rec[28000] == \
    [7033, 17125, 15462, 33049, 51105, 16424, 23, 7996, 725, 1539]
assert ID_influence_rec[29000] == \
    [28606]
assert ID_influence_rec[30000] == \
...
# In problem 8 two lists are created.  One stores the IDs of the people
# whos lists are the same, no matter the method, and the other stores
# the IDs of people that have different recommendations.  The number of 
# people in each list are printed.
changed = []
unchanged = []
for userID in ID_by_1000:
    if ID_influence_rec[userID] == ID_common_rec[userID]:
        unchanged.append(userID)
    if ID_influence_rec[userID] != ID_common_rec[userID]:
        changed.append(userID)
 
print "Same:", len(unchanged)
print "Different:", len(changed)
